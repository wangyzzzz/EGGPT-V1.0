...
def objective(trial):
    ...
    min_samples_split = trial.suggest_categorical('min_samples_split', np.arange(2, 11, 2))
    min_samples_leaf = trial.suggest_categorical('min_samples_leaf', [1, 2, 4, 6])  # Python 列表也可以
    ...
    for fold in range(int(k_fold)):
        ...
        record['min_samples_split'].append(min_samples_split)
        record['min_samples_leaf'].append(min_samples_leaf)
        record['max_depth'].append(max_depth)
        record['max_features'].append(max_features)
        record['fold'].append(fold)
        # print (n_estimators[0], min_samples_split[0], min_samples_leaf[0], max_depth[0], max_features[0], batch_size)
        print (n_estimators, min_samples_split, min_samples_leaf, max_depth, max_features)
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            train_rf_pred = rf.predict(train_inputs)

            rf_r, rf_p_value = pearsonr(train_labels, train_rf_pred)
            rf_r2 = r2_score(train_labels, train_rf_pred)
            rf_rmse = np.sqrt(mean_squared_error(train_rf_pred, train_labels))
            # print ("train_RF r2: ", rf_r2, " r: ", rf_r, " rmse: ", rf_rmse)
        ...
        for i, data in enumerate(test_dataloader, 0):   
            ...
            rf_r, rf_p_value = pearsonr(labels, rf_pred)
            rf_r2 = r2_score(labels, rf_pred)
            rf_rmse = np.sqrt(mean_squared_error(rf_pred, labels))

            print ("RF r2: ", rf_r2, " r: ", rf_r, " rmse: ", rf_rmse)
        record['loss'].append(1-rf_r2)
        record['R2'].append(rf_r2)
        record['R'].append(0)
        record['RMSE'].append(rf_rmse)
        # record['epoch'].append(record_epoch)

        losses.append(1-rf_r2)
        R2s.append(rf_r2)
        ...
        original_rf_predict = inverse_standardize(rf_pred, data_mean, data_std)
        # now = int(time.time())
    
        test_df = pd.DataFrame({
            'original_real': original_real,
            # 'original_svr_predict': original_svr_predict,
            'original_predict': original_rf_predict,
            'standard_real': labels, 
            # 'standard_svr_predict': svr_pred,
            'original_predict': rf_pred,
            'gene_id': ids,
        })

        standard_train_rf = rf.predict(train_inputs)
    ...
    if (np.mean(R2s) > now_r2):
        now_r2 = np.mean(R2s)
        for i in range(int(k_fold)):
            total_save_url = root_path + 'Result/' + str(trait) + '/Base_model_result/RF/'
            os.makedirs(root_path + 'Result/' + str(trait) + '/Base_model_result/RF/', exist_ok=True)
...
df_info.to_csv(root_path + 'Result/optuna/'+trait+'_'+save_result_name+ '_RF_'+str(data_format)+'_' + str(gene_model)+'_record.csv', index_label='key')

# 保存Study对象
joblib.dump(study, root_path + 'Result/optuna/'+trait+'_'+save_result_name + '_RF_'+str(data_format)+'_' + str(gene_model)+'.pkl')
