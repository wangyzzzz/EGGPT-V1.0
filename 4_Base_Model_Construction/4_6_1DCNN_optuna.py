...
# 定义模型
class CNN1D_PyTorch(nn.Module):
    def __init__(self, n, activation_name="ReLU", hidden_units=64, kernel_size=4, dropout1=0.2, lr=1e-3):
        super(CNN1D_PyTorch, self).__init__()
        self.net = nn.Sequential(
            nn.Conv1d(1, hidden_units, kernel_size=kernel_size, padding='same'),  # 输入通道数为 1
            getattr(nn, activation_name)(),
            nn.Dropout(dropout1),
            nn.BatchNorm1d(hidden_units),
            nn.Conv1d(hidden_units, hidden_units, kernel_size=kernel_size, padding='same'),
            getattr(nn, activation_name)(),
            nn.Dropout(dropout1),
            nn.BatchNorm1d(hidden_units),
            nn.Conv1d(hidden_units, hidden_units, kernel_size=kernel_size, padding='same'),
            ...
            nn.BatchNorm1d(hidden_units),
            nn.Flatten(),
            nn.Linear(hidden_units * n, 3), # n 是输入序列长度
            nn.Dropout(dropout1),
            nn.Linear(3, 1)
        )
...
# gene, label = train_dataset.__getitem__(0)
# gene_count = len(gene)
...
def objective(trial):
    ...
    if gene_model == 'PCA' or gene_model == 'GE_PCA':
        ...
        hidden_units = trial.suggest_categorical('hidden_units', [2, 4, 8, 16, 32])
    ...
    # lr = trial.suggest_categorical('lr', [5e-5, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2])
    ...
    for fold in range(int(k_fold)):
        ...
        model = CNN1D_PyTorch(n=gene_count_array[fold], activation_name=activation_name, hidden_units=hidden_units, dropout1=dropout_rate, lr=lr, kernel_size=kernel_size)
        ...
        min_loss, min_rmse, max_r2, max_r = 5000, 100000, -1000, -1000
    ...
    if (np.mean(R2s) > now_r2):
        now_r2 = np.mean(R2s)
        for i in range(int(k_fold)):
            total_save_url = root_path + 'Result/' + str(trait) + '/Base_model_result/CNN/'
...
import os
os.makedirs(root_path + 'Result/' + str(trait) + '/Base_model_result/CNN/temp', exist_ok=True)
...
df_info.to_csv(root_path + 'Result/optuna/'+trait+'_'+save_result_name+ '_CNN_'+str(data_format)+'_' + str(gene_model)+'_record.csv', index_label='key')

# 保存Study对象
joblib.dump(study, root_path + 'Result/optuna/'+trait+'_'+save_result_name + '_CNN_'+str(data_format)+'_' + str(gene_model)+'.pkl')
